...
class MoviesRepository:
    ...
    def get_producers_by_prize_ranges(self):
        ...
        for row in cursor.fetchall():
            ...
            for name in names:
                producers_list.append({"name": name, "year": row[1]})
        ...
        for producer in producers_list:
            if last_name == producer["name"]:
                ...
                if producer_with_longest_gap_between_two_awards["interval"] < interval:
                    producer_with_longest_gap_between_two_awards["producer"] = producer[
                        "name"
                    ]
        ...
        return {
            "min": [producer_got_2_awards_faster],
            "max": [producer_with_longest_gap_between_two_awards],
        }
